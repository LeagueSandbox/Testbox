...
ServerLogic.prototype.updateAllGameServers = function() {
    for (var i = 0; i < this.gameServers.length; i++) {
        ...
        this.updateGameServer(gs['repository'], gs['branch'], "", false, function(){
            console.log("Success updating server!");
        });
    }
};

ServerLogic.prototype.updateGameServer = function(repository, branch, gameJSON, needsCopied, callback) {
...
ServerLogic.prototype.startGameServer = function(repository, branch, gameJSON, port, callback) {
    this.updateGameServer(repository, branch, gameJSON, true, CreateFunction(this, function (serverName) {
        const exec = require('child_process').spawn;
